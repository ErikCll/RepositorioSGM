...
namespace Operacion.Clase
{
    public class Disponibilidad
    {
        ...
        public DataTable MostrarEquipos(string txtSearch,int IdIntalacion)
        {
            ...
            string query = "SELECT equi.Id_Equipo,equi.Nombre,equi.Id_Instalacion, bitc.maxId, CONVERT(nvarchar,bitc2.ini_averia, 105) 'Fecha', CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END 'Estatus', FORMAT(bitc2.ini_averia, 'hh:mm tt') 'Hora' FROM Cat_Equipo equi LEFT JOIN(SELECT Id_Equipo, MAX(Id_Falla) maxId FROM Op_Fallas WHERE Activado IS NULL GROUP BY Id_Equipo) bitc on equi.Id_Equipo = bitc.Id_Equipo LEFT JOIN(SELECT Id_Falla, Id_Equipo, ini_averia, fin_averia FROM Op_Fallas) bitc2 on bitc.maxId = bitc2.Id_Falla WHERE equi.Id_Instalacion = @IdInstalacion AND equi.Activado IS NULL ORDER BY bitc.maxId DESC, equi.Id_Equipo DESC";
            if (!String.IsNullOrEmpty(txtSearch.Trim()))
            {
                query = "SELECT equi.Id_Equipo,equi.Nombre,equi.Id_Instalacion, bitc.maxId, CONVERT(nvarchar,bitc2.ini_averia, 105) 'Fecha', CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END 'Estatus', FORMAT(bitc2.ini_averia, 'hh:mm tt') 'Hora' FROM Cat_Equipo equi LEFT JOIN(SELECT Id_Equipo, MAX(Id_Falla) maxId FROM Op_Fallas WHERE Activado IS NULL GROUP BY Id_Equipo) bitc on equi.Id_Equipo = bitc.Id_Equipo LEFT JOIN(SELECT Id_Falla, Id_Equipo, ini_averia, fin_averia FROM Op_Fallas) bitc2 on bitc.maxId = bitc2.Id_Falla WHERE equi.Id_Instalacion = @IdInstalacion AND equi.Activado IS NULL AND equi.Nombre LIKE '%' + @txtSearch + '%' ORDER BY bitc.maxId DESC, equi.Id_Equipo DESC";
            }
