...
namespace Operacion.Clase
{
    public class Disponibilidad
    {
        ...
        public DataTable MostrarEquipos(string txtSearch,int IdIntalacion)
        {
            ...
            string query = "SELECT equi.Id_Equipo,equi.Nombre,equi.Id_Instalacion, bitc.maxId, CONVERT(nvarchar,bitc2.ini_averia, 105) 'Fecha', CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END 'Estatus', FORMAT(bitc2.ini_averia, 'hh:mm tt') 'Hora',CASE WHEN CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END ='Falla' THEN CAST(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE())) / 1440 AS VARCHAR(8))+'d,'+CAST(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE()))% 1440 / 60 AS VARCHAR(8)) + 'h:' + FORMAT(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE())) % 60, 'D2')+'m'ELSE NULL END 'Transcurrido',CASE WHEN bitc2.Id_TipoFalla=0 THEN 'Otra' ELSE falla.Nombre END 'TipoFalla' FROM Cat_Equipo equi LEFT JOIN(SELECT Id_Equipo, MAX(Id_Falla) maxId FROM Op_Fallas WHERE Activado IS NULL AND CASE WHEN DATEDIFF(minute, ini_averia, fin_averia) IS NULL THEN '1010' ELSE DATEDIFF(minute, ini_averia, fin_averia) END>2 GROUP BY Id_Equipo) bitc on equi.Id_Equipo = bitc.Id_Equipo LEFT JOIN(SELECT Id_Falla, Id_Equipo, ini_averia, fin_averia,Id_TipoFalla FROM Op_Fallas) bitc2 on bitc.maxId = bitc2.Id_Falla LEFT JOIN Cat_Falla falla on bitc2.Id_TipoFalla=falla.Id_Falla  WHERE equi.Id_Instalacion = @IdInstalacion AND equi.Activado IS NULL ORDER BY Estatus,Transcurrido, Id_equipo ASC";
            if (!String.IsNullOrEmpty(txtSearch.Trim()))
            {
                query = "SELECT equi.Id_Equipo,equi.Nombre,equi.Id_Instalacion, bitc.maxId, CONVERT(nvarchar,bitc2.ini_averia, 105) 'Fecha', CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END 'Estatus', FORMAT(bitc2.ini_averia, 'hh:mm tt') 'Hora', CASE WHEN CASE WHEN bitc.maxId IS NULL OR bitc2.fin_averia IS NOT NULL THEN 'Operando' ELSE 'Falla' END ='Falla' THEN CAST(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE())) / 1440 AS VARCHAR(8))+'d,'+CAST(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE()))% 1440 / 60 AS VARCHAR(8)) + 'h:' + FORMAT(DATEDIFF(minute, ini_averia, DATEADD(HH, -5, GETDATE())) % 60, 'D2')+'m'ELSE NULL END 'Transcurrido',CASE WHEN bitc2.Id_TipoFalla=0 THEN 'Otra' ELSE falla.Nombre END 'TipoFalla' FROM Cat_Equipo equi LEFT JOIN(SELECT Id_Equipo, MAX(Id_Falla) maxId FROM Op_Fallas WHERE Activado IS NULL AND CASE WHEN DATEDIFF(minute, ini_averia, fin_averia) IS NULL THEN '1010' ELSE DATEDIFF(minute, ini_averia, fin_averia) END>2 GROUP BY Id_Equipo) bitc on equi.Id_Equipo = bitc.Id_Equipo LEFT JOIN(SELECT Id_Falla, Id_Equipo, ini_averia, fin_averia,Id_TipoFalla FROM Op_Fallas) bitc2 on bitc.maxId = bitc2.Id_Falla LEFT JOIN Cat_Falla falla on bitc2.Id_TipoFalla=falla.Id_Falla  WHERE equi.Id_Instalacion = @IdInstalacion AND equi.Activado IS NULL AND equi.Nombre LIKE '%' + @txtSearch + '%' ORDER BY Estatus,Transcurrido, Id_equipo ASC";
            }
