...
namespace SGM.Clase
{
    public class ProgramaCapacitacion
    {
        ...
        public DataTable MostrarAnios(int IdInstalacion)
        {
            ...
            comm.Connection = conexion.AbrirConexion();
            comm.CommandText = "SELECT YEAR(prog.FechaEvaluacion) 'Anio' FROM Op_ProgramaCapacitacion prog JOIN Evaluacion ev on prog.Id_Evaluacion = ev.Id_Evaluacion JOIN Cat_ActividadControl ctr on ev.Id_Control = ctr.Id_Control JOIN Cat_Actividades act on ctr.Id_Actividad = act.Id_Actividades JOIN Cat_Area area on act.Id_Area = area.Id_area JOIN Cat_Instalacion ins on area.Id_instalacion = ins.Id_instalacion WHERE prog.Activado IS NULL AND ins.Id_instalacion = @IdInnstalacion GROUP BY YEAR(prog.FechaEvaluacion) ORDER BY YEAR(prog.FechaEvaluacion) DESC";
            comm.CommandType = CommandType.Text;
            comm.Parameters.AddWithValue("@IdInnstalacion", IdInstalacion);
