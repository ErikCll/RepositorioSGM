...
namespace SASISOPA.Clase
{
    public class CategoriaEmpleado
    {
        ...
        public DataTable Mostrar(string txtSearch, int IdInstalacion)
        {
            ...
            {
                query = "SELECT cat.Id_Categoria,cat.Nombre,area.Nombre 'Area' FROM Cat_Categoria cat JOIN Cat_Area area on cat.Id_Area = area.Id_area Cat_Instalacion ins on area.Id_instalacion = ins.Id_instalacion WHERE cat.Activado IS NULL AND ins.Id_instalacion =@IdInstalacion AND cat.Nombre LIKE '%'+@txtSearch+'%' ORDER BY cat.Id_Categoria DESC";
            }
