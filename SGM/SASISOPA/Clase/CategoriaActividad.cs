...
namespace SASISOPA.Clase
{
    public class CategoriaActividad
    {
        ...
        public DataTable MostrarActividades(int IdCategoria,int IdInstalacion)
        {
            ...
            string query = "SELECT act.Id 'Id_Actividad', act.Nombre,ins.Nombre 'Instalacion', CAST(CASE WHEN CatAct.Id_Actividad IS NULL THEN 0 else CatAct.Id_Actividad END as bit) 'Id_registro',  CASE WHEN CatAct.Id_Actividad IS NULL THEN 0 else CatAct.Id_Actividad END 'Id_registro2' FROM (SELECT InsAct.Id, act.Id_Actividades, act.Nombre,InsAct.Id_Instalacion FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades WHERE InsAct.Id_Instalacion = @IdInstalacioon AND act.Activado IS NULL AND act.TipoSistema = 2) act LEFT JOIN(SELECT Id_Actividad FROM Op_Cat_Act WHERE Id_Categoria = @Id_Categoria) CatAct on act.Id = CatAct.Id_Actividad JOIN Cat_Instalacion ins on act.Id_Instalacion=ins.Id_instalacion ORDER BY Id_registro DESC";
