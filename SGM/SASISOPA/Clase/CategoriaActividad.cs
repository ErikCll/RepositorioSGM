...
namespace SASISOPA.Clase
{
    public class CategoriaActividad
    {
        ...
        public DataTable MostrarGeneral(int IdInstalacion)
        {
            ...
            string query = "DECLARE @cols AS NVARCHAR(MAX), @query AS NVARCHAR(MAX) DECLARE @Id_Instalacion as NVARCHAR(MAX)set @Id_Instalacion = @IdInstalacion select @cols = STUFF((SELECT ',' + QUOTENAME(cat.Nombre) FROM Cat_Categoria cat JOIN Cat_Area area on cat.Id_Area = area.Id_area JOIN Cat_Instalacion ins on area.Id_instalacion = ins.Id_instalacion WHERE ins.Id_instalacion = @Id_Instalacion AND cat.Activado IS NULL FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'),1,1,'') set @query = 'SELECT Actividad,Area as Área,' + @cols + ' from (select actividad,Area, Categoria FROM(SELECT act.nombre Actividad, act.Area, cat.Nombre Categoria FROM Op_Cat_Act catact LEFT JOIN Cat_Categoria cat on catact.Id_Categoria = cat.Id_Categoria RIGHT JOIN(SELECT InsAct.Id, act.Nombre, area.Id_area, area.Nombre AS Area FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades JOIN Cat_Area area on InsAct.Id_Area = area.Id_area WHERE InsAct.Id_Instalacion = '+@Id_Instalacion+' AND act.Activado IS NULL AND act.TipoSistema = 2) act on catact.Id_Actividad = act.Id WHERE cat.Activado IS NULL) as tabla) x pivot (MAX(Categoria) for Categoria in (' + @cols + ') ) p 'execute(@query);";
        ...
        public DataTable MostrarActividades(int IdCategoria,int IdArea,int IdInstalacion)
        {

            string query = "SELECT act.Id 'Id_Actividad', act.Nombre, area.Nombre 'Area',  CAST(CASE WHEN CatAct.Id_Actividad IS NULL THEN 0 else CatAct.Id_Actividad END as bit) 'Id_registro', CASE WHEN CatAct.Id_Actividad IS NULL THEN 0 else CatAct.Id_Actividad END 'Id_registro2' FROM (SELECT InsAct.Id, act.Id_Actividades, act.Nombre, area.Id_Area, area.Nombre 'Area' FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades JOIN Cat_Area area on InsAct.Id_Area = area.Id_area WHERE InsAct.Id_Instalacion = @IdInstalacioon AND act.Activado IS NULL AND act.TipoSistema = 2) act LEFT JOIN(SELECT Id_Actividad FROM Op_Cat_Act WHERE Id_Categoria = @Id_Categoria) CatAct on act.Id = CatAct.Id_Actividad JOIN Cat_Area area on act.Id_Area = area.Id_Area WHERE area.Id_Area = @IdArea ORDER BY Id_registro DESC, act.Id_Area DESC";
