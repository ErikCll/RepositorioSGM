...
namespace SAM.Clase
{
    public class Master
    {
        ...
        public DataTable MostrarInstalacion(int IdSuscripcion, int IdUsuario)
        {
            ...
            comm.CommandText = "SELECT Nav.Id_instalacion, Nav.Nombre  FROM Cat_Instalacion Nav JOIN(SELECT Id_Instalacion FROM Op_UsIns WHERE Id_Usuario=@IdUsuario) UsAct on Nav.Id_instalacion = UsAct.Id_Instalacion WHERE nav.Activado IS NULL AND nav.Id_Suscripcion = @MIdSuscripcion ORDER BY nav.Id_instalacion DESC";
