...
namespace SAM.Clase
{
    public class Cliente
    {
        ...
        public DataTable MostrarInstalacion(int IdSuscripcion,int IdUsuario)
        {
            ...
            comm.Connection = conexion.AbrirConexion();
            comm.CommandText = "SELECT Nav.Id_instalacion, Nav.Nombre  FROM Cat_Instalacion Nav  JOIN(SELECT Id_Instalacion FROM Op_UsIns op WHERE Id_Usuario=@IdUsuario)UsAct on Nav.Id_instalacion = UsAct.Id_Instalacion WHERE nav.Activado IS NULL AND nav.Id_Suscripcion = @IdSuscripcionn ORDER BY nav.Id_instalacion DESC";
            comm.CommandType = CommandType.Text;
