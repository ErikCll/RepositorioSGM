...
namespace SGL.Clase
{
    public class InstalacionActividad
    {
        ...
        public DataTable MostrarActividades(int IdInstalacion,int IdSuscripcion)
        {
            ...
            string query = "SELECT act.Id_Actividades 'Id_Actividad', act.Nombre,ISNULL(r.max_score, '0') 'Archivo', CAST(CASE WHEN InsAct.Id_Actividad IS NULL THEN 0 else InsAct.Id_Actividad END as bit) 'Id_registro', CASE WHEN InsAct.Id_Actividad IS NULL THEN 0 else InsAct.Id_Actividad END 'Id_registro2' FROM Cat_Actividades act LEFT JOIN(SELECT Id_Actividad, Id_Area FROM Op_Ins_Act WHERE Id_Instalacion = @IdInsstalacion) InsAct on act.Id_Actividades = InsAct.Id_Actividad LEFT JOIN(SELECT Id_Actividad, MAX(Id_Control) max_score FROM Cat_ActividadControl WHERE Activado IS NULL GROUP BY Id_Actividad) r on act.Id_Actividades = r.Id_Actividad   WHERE act.Activado IS NULL AND act.TipoSistema = 3 AND act.Id_Suscripcion=@IdSuscripcionn ORDER BY Id_registro DESC, act.Id_Actividades DESC";
