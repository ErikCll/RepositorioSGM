...
namespace SGL.Clase
{
    public class Actividad
    {
        ...
        public DataTable MostrarGrafica(int IdInstalacion)
        {
            ...
            string query = "SELECT t1.Total,t2.Disponibles,t3.Vigentes,t4.Evaluacion  FROM(SELECT COUNT(*) 'Total','1' 'Id' FROM(SELECT InsAct.Id, act.Id_Actividades, act.Nombre, InsAct.Id_Instalacion FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades WHERE InsAct.Id_Instalacion = @IdInstalacion AND act.Activado IS NULL AND act.TipoSistema = 3) act LEFT JOIN(SELECT Id_Actividad, MAX(Id_Control) max_score FROM Cat_ActividadControl WHERE Activado IS NULL GROUP BY Id_Actividad) r on act.Id_Actividades = r.Id_Actividad LEFT JOIN(SELECT Id_Control, FechaEmision, Codigo, VigenciaMeses FROM Cat_ActividadControl) s on r.max_score = s.Id_Control JOIN Cat_Instalacion ins on act.Id_instalacion = ins.Id_instalacion WHERE ins.Id_Instalacion = @IdInstalacion GROUP BY ins.Id_instalacion)t1 JOIN (SELECT COUNT(*) 'Disponibles', '1' 'Id' FROM(SELECT InsAct.Id, act.Id_Actividades, act.Nombre, InsAct.Id_Instalacion FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades WHERE InsAct.Id_Instalacion = @IdInstalacion AND act.Activado IS NULL AND act.TipoSistema = 3) act LEFT JOIN(SELECT Id_Actividad, MAX(Id_Control) max_score FROM Cat_ActividadControl WHERE Activado IS NULL GROUP BY Id_Actividad) r on act.Id_Actividades = r.Id_Actividad LEFT JOIN(SELECT Id_Control, FechaEmision, Codigo, VigenciaMeses FROM Cat_ActividadControl) s on r.max_score = s.Id_Control JOIN Cat_Instalacion ins on act.Id_instalacion = ins.Id_instalacion WHERE ins.Id_Instalacion = @IdInstalacion AND s.Id_Control IS NOT NULL GROUP BY ins.Id_instalacion)t2 on t1.Id = t2.Id JOIN (SELECT COUNT(*) 'Vigentes', '1' 'Id' FROM(SELECT InsAct.Id, act.Id_Actividades, act.Nombre, InsAct.Id_Instalacion FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades WHERE InsAct.Id_Instalacion = @IdInstalacion AND act.Activado IS NULL AND act.TipoSistema = 3) act LEFT JOIN(SELECT Id_Actividad, MAX(Id_Control) max_score FROM Cat_ActividadControl WHERE Activado IS NULL GROUP BY Id_Actividad) r on act.Id_Actividades = r.Id_Actividad LEFT JOIN(SELECT Id_Control, FechaEmision, Codigo, VigenciaMeses, Activado FROM Cat_ActividadControl) s on r.max_score = s.Id_Control JOIN Cat_Instalacion ins on act.Id_instalacion = ins.Id_instalacion WHERE ins.Id_Instalacion = @IdInstalacion AND s.Id_Control IS NOT NULL AND CASE WHEN CONVERT(DATE, DATEADD(HOUR, -5, GETDATE())) < CONVERT(DATE, DATEADD(MONTH, ISNULL(s.VigenciaMeses, '0'), s.FechaEmision)) OR ISNULL(s.VigenciaMeses, '0') = 0 THEN 'Vigente' WHEN CONVERT(DATE, DATEADD(HOUR, -5, GETDATE())) >= CONVERT(DATE, DATEADD(MONTH, ISNULL(s.VigenciaMeses, '0'), s.FechaEmision)) THEN 'Vencido' END = 'Vigente' GROUP BY ins.Id_instalacion)t3 on t2.Id = t3.Id JOIN (SELECT COUNT(*) 'Evaluacion', '1' 'Id' FROM(SELECT InsAct.Id, act.Id_Actividades, act.Nombre, InsAct.Id_Instalacion FROM Op_Ins_Act InsAct JOIN Cat_Actividades act on InsAct.Id_Actividad = act.Id_Actividades WHERE InsAct.Id_Instalacion = @IdInstalacion AND act.Activado IS NULL AND act.TipoSistema = 3) act LEFT JOIN(SELECT Id_Actividad, MAX(Id_Control) max_score FROM Cat_ActividadControl WHERE Activado IS NULL GROUP BY Id_Actividad) r on act.Id_Actividades = r.Id_Actividad LEFT JOIN(SELECT Id_Control, FechaEmision, Codigo, VigenciaMeses FROM Cat_ActividadControl) s on r.max_score = s.Id_Control JOIN Cat_Instalacion ins on act.Id_instalacion = ins.Id_instalacion LEFT JOIN Evaluacion ev on s.Id_Control = ev.Id_Control AND ev.Activado IS NULL AND ev.Estatus = 2 WHERE ins.Id_Instalacion = @IdInstalacion AND ev.Id_Evaluacion IS NOT NULL GROUP BY ins.Id_instalacion)t4 on t3.Id = t4.Id";
