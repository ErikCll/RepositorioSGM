...
namespace SGL.Clase
{
    public class EMA
    {
        ...
        public DataTable Mostrar(string txtSearch, int IdInstalacion)
        {
            ...
            {
                query = "SELECT acre.Id_Acreditacion,acre.Numero 'No',acre.Referencia,CONVERT(NVARCHAR,sta2.Fecha,105) 'Fecha',sta2.Status,ISNULL(sta.Id_status,0) 'Id_status' FROM Op_Acreditacion acre LEFT JOIN(SELECT MAX(Id_Status) 'Id_status', Id_acreditacion FROM Op_AcreditacionStatus WHERE Activado IS NULL GROUP BY Id_Acreditacion) sta on acre.Id_Acreditacion = sta.Id_Acreditacion LEFT JOIN Op_AcreditacionStatus sta2 on sta.Id_status=sta2.Id_Status WHERE acre.Id_Instalacion = @IdInstalacion AND acre.Activado IS NULL AND acre.Acreditador = 'EMA' AND acre.No LIKE '%'+@txtSearch+'%' ORDER BY acre.Id_Acreditacion DESC ";
            }
